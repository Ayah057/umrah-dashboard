...
def get_7_day_comparison(test_df, selected_month, selected_day):
    ...
    # إذا في أكثر من سنة، خذ السنة الأحدث فقط
    if "Hijri_Year" in comparison_df.columns:
        latest_year = comparison_df["Hijri_Year"].max()
        comparison_df = comparison_df[comparison_df["Hijri_Year"] == latest_year].copy()
    elif DATE_COL in comparison_df.columns:
        latest_year = comparison_df[DATE_COL].dt.year.max()
        comparison_df = comparison_df[comparison_df[DATE_COL].dt.year == latest_year].copy()

    # تأكد إنه 7 أيام بحد أقصى
