...
def add_display_cols(df):
    ...
    import re as _re
    def _extract_day_fixed(val):
        if pd.isna(val): return np.nan
        m = _re.match(r"^(\d{1,2})[-/](\d{1,2})[-/](\d{4})$", str(val).strip())
        if m: return int(m.group(1))
        return extract_hijri_day(val)
    
    df["Hijri_Day_Num"] = df["Hijri_Date"].apply(_extract_day_fixed)
    ...
    # Fix Hijri_Day_Num from Hijri_Date format DD-MM-YYYY
    if df["Hijri_Day_Num"].isna().all():
        ...
        def extract_day(val):
            ...
            if m: return int(m.group(1))
            return np.nan
        df["Hijri_Day_Num"] = df["Hijri_Date"].apply(extract_day)
    ...
    if model_path is not None:
        ...
        if isinstance(package, dict):
            if data_file is not None:
                package["full_df"] = pd.read_excel(data_file)
            return package
        return {"model": package}

    if data_file is not None:
        return {"full_df": pd.read_excel(data_file)}
...
def get_test_df_from_package(package):
    # Always prefer full excel data (both years)
    if "full_df" in package:
        return add_display_cols(package["full_df"])
...
def get_7_day_comparison(test_df, selected_month, selected_day):
    ...
    if month_df.empty:
        return month_df, start_day, end_day

    if "Hijri_Year" in month_df.columns:
        latest_year = month_df["Hijri_Year"].max()
        month_df = month_df[month_df["Hijri_Year"] == latest_year].copy()

    comparison_df = month_df[
        (month_df["Hijri_Day_Num"] >= start_day) &
        (month_df["Hijri_Day_Num"] <= end_day)
    ].copy()

    if comparison_df.empty:
        month_df = month_df.sort_values("Hijri_Day_Num")
        comparison_df = month_df.head(7).copy()
        if not comparison_df.empty:
            start_day = int(comparison_df["Hijri_Day_Num"].iloc[0])
            end_day = int(comparison_df["Hijri_Day_Num"].iloc[-1])

    comparison_df = comparison_df.head(7)
...
def input_page():
    with st.form("prediction_form"):
        ...
        year = st.selectbox("السنة الهجرية", [1445, 1446], index=1)
...
def dashboard_page():
    ...
    selected_year = st.session_state.get("selected_year", 1446)
    ...
    if "Hijri_Year" in test_df.columns:
        test_df = test_df[test_df["Hijri_Year"] == selected_year].copy()
